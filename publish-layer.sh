...
DOCKERFILE="./build/chromedriver-layer-builder/dockerfile"
LAYER_NAME=$(cat ${DOCKERFILE} | grep "^ARG LAYER_NAME=" | sed -r 's#.*=(.*)#\1#')
ZIP_FILE_NAME="${LAYER_NAME}.zip"
ZIP_FILE_LOC="./layers/${ZIP_FILE_NAME}"
CHROME_BUILD_NUMBER=$(cat ${DOCKERFILE} | grep "^ARG CHROME_BUILD=" | sed -r 's#.*=(.*)#\1#')
...
publish_layer() {
  ...
  local output=$(aws lambda publish-layer-version \
    ...
    --description "Chrome Build ${CHROME_BUILD_NUMBER}" \
